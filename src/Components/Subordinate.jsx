...
function Subordinate({ id }) {
    ...
    return (
        <div className="card p-3 mb-3 Subordinate">
            <div className={`${Boolean(memberList?.length || subBranchesList?.length) ? "pb-3" : ""}`}>
                <div className="d-flex align-items-center position-relative">
                    <h6 className="card-title mb-0">Subordinate {id}</h6>
                    <FaCirclePlus style={{fontSize: "20px" , color: "green", cursor: "pointer"}} onClick={addMember} className="ms-2"/>
                    ...
                    <ul class="dropdown-menu">
                        <li><div className="px-2" style={{cursor: "pointer"}} onClick={addSubBranch}>Add a New Subordinate Branch</div></li>
                    </ul>
                </div>
            </div>
            {/* <div className="mb-2">
